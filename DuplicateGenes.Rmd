...
medLogMeanRnks <- 
  ...
  map2_df(grpSize, orthoRankTables, .id="spcPair",
        ~ .x %>% 
          unnest(SPC1) %>% 
          inner_join(.y, by = "SPC1")) %>% 
  # remove NA's and keep only 1:N and N:1 orthologs where N=1..4
  filter( !is.na(rnks), (N1==1 & N2<5) | (N2==1 & N1<5)) %>% 
  # calculate mean of both directions
  mutate( meanRnk = (rnks+rnksT)/2 ) %>% 
  # calculate median of ranks
  group_by(spcPair,N1,N2) %>% 
  summarise( medRnk = median(-log10(1.0001-meanRnk))) %>% 
  ungroup %>% 
...
medLogMeanRnks %>% 
  ...
  ggplot( aes(x=pmax(N1,N2),y=medRnk,color=spcPair2) ) +
